...
export function EventsGrid({ initialItems }: EventsGridProps) {
  const [items] = useState<Event[]>(initialItems);
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        {/* Search */}
        <div className="relative flex-1 max-w-7xl mx-auto border border-primary/80 rounded-lg mb-6">
      ...
      {filteredItems.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item, idx) => {
            ...
            return (
              <Link href={`/events/${item.slug || item.id}`} key={item.id} className="block group">
                <motion.div
