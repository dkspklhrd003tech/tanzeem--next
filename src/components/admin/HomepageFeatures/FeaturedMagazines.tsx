...
function SortableMagazineRow({ magazine, onEdit, onDelete }: { magazine: any; onEdit: (m: any) => void; onDelete: (id: string, title: string) => void }) {
    const {
        attributes,
        listeners,
        setNodeRef,
        ...
        <tr ref={setNodeRef} style={style} className="hover:bg-muted/30 group">
            ...
            <td className="py-3">
                <div className="w-12 h-16 bg-muted border border-border flex items-center justify-center overflow-hidden">
                    {magazine.coverImage ? <img src={magazine.coverImage} className="w-full h-full object-cover" /> : <ImageIcon className="w-4 h-4 text-muted-foreground" />}
                </div>
            </td>
            ...
            <td className="py-3 text-right">
                <button onClick={() => onEdit(magazine)} className="p-2 text-foreground-light hover:text-primary"><Edit2 className="w-4 h-4" /></button>
                <button onClick={() => onDelete(magazine.id, magazine.title)} className="p-2 text-foreground-light hover:text-destructive"><XCircle className="w-4 h-4" /></button>
            </td>
        </tr>
    );
}
...
export function FeaturedMagazines() {
    ...
    const fetchMagazines = async () => {
        try {
            const res = await fetch("/api/magazines"); // Or with featured filter if supported
            if (res.ok) {
    ...
    const handleDragEnd = async (event: any) => {
        ...
        if (active.id !== over.id) {
            const oldIndex = magazines.findIndex((m) => m.id === active.id);
            const newIndex = magazines.findIndex((m) => m.id === over.id);

            const newOrder = arrayMove(magazines, oldIndex, newIndex);
            setMagazines(newOrder);

            // Sync with backend
            try {
                const patchPayload = newOrder.map((m, index) => ({
                    id: m.id,
                    order: index,
                }));
    ...
    const handleDelete = async (id: string, title: string) => {
        ...
        try {
            ...
            toast({
                title: "Error",
                description: err.message || "Failed to remove magazine record.",
                variant: "destructive"
            });
        }
    };
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center pb-4 border-b border-border mb-6">
                ...
                <button onClick={() => handleOpenModal()} className="flex items-center gap-2 bg-primary text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm hover:shadow-md active:scale-95">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
                    <div className="w-full max-w-md bg-card border border-border rounded-xl shadow-xl overflow-hidden relative">
                        <div className="p-6 border-b border-border flex justify-between items-center bg-muted/30">
                            <h3 className="font-bold text-xl text-foreground">{editingItem ? "Edit Magazine Details" : "Add New Magazine"}</h3>
                        ...
                        <form id="magazine-form" onSubmit={handleSave} className="p-6 space-y-6 bg-card">
                            <div className="space-y-3">
                                <label className="text-sm font-semibold text-foreground flex items-center gap-2">
                                    ...
                                    Magazine Cover <span className="text-xs font-normal text-foreground-muted">(240x332 Size recommended)</span>
                                </label>
                                <ImageUploader
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full py-2.5 border border-border rounded-xl bg-background text-foreground focus:ring-2 focus:ring-primary/20 transition-all outline-none"
                                    placeholder="e.g. Issue #42 or Current Issue"
                                />
                            </div>

                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-foreground">Magazine Title</label>
                                <input type="text" required value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className="w-full py-2.5 border border-border rounded-xl bg-background text-foreground focus:ring-2 focus:ring-primary/20 transition-all outline-none" placeholder="Enter magazine title" />
                            </div>
                            <div className="flex gap-4">
                                <div className="flex-1 space-y-2">
                                    <label className="text-sm font-semibold text-foreground">Frequency / Subtitle</label>
                                    <input type="text" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} className="w-full py-2.5 border border-border rounded-xl bg-background text-foreground focus:ring-2 focus:ring-primary/20 transition-all outline-none" placeholder="e.g. Monthly" />
                                </div>
                            ...
                            <div className="flex gap-4">
                                <div className="flex-1 space-y-2">
                                    ...
                                    <input type="text" value={formData.buttonText} onChange={e => setFormData({ ...formData, buttonText: e.target.value })} className="w-full py-2.5 border border-border rounded-xl bg-background text-foreground focus:ring-2 focus:ring-primary/20 transition-all outline-none" placeholder="e.g. More Nida e Khilafat" />
                                </div>
                                <div className="flex-1 space-y-2">
                                    <label className="text-sm font-semibold text-foreground">Button URL (Optional)</label>
                                    <input type="text" value={formData.buttonUrl} onChange={e => setFormData({ ...formData, buttonUrl: e.target.value })} className="w-full py-2.5 border border-border rounded-xl bg-background text-foreground focus:ring-2 focus:ring-primary/20 transition-all outline-none" placeholder="e.g. /magazines/nida-e-khilafat" />
                                </div>
                            </div>
