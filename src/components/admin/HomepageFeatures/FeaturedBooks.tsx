    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-foreground">Featured Books List</h2>
                        ...
                        <tr key={b.id} className="hover:bg-muted/30">
                            <td className="px-4 py-3">
                                <div className="w-12 h-16 bg-muted border border-border flex items-center justify-center overflow-hidden">
                                    {b.coverImage ? <img src={b.coverImage} className="w-full h-full object-cover" /> : <ImageIcon className="w-4 h-4 text-muted-foreground" />}
                                </div>
                            </td>
                            <td className="px-4 py-3 font-medium text-foreground">
                                {b.title}
                                <div className="text-xs text-foreground-muted font-normal">{b.authorName}</div>
                            </td>
                            <td className="px-4 py-3">
                                <Badge variant={b.isFeatured ? "default" : "secondary"}>{b.isFeatured ? "Featured" : "Hidden"}</Badge>
                            </td>
                            <td className="px-4 py-3 text-right">
                                <button onClick={() => handleOpenModal(b)} className="p-2 text-foreground-light hover:text-primary"><Edit2 className="w-4 h-4" /></button>
                                <button onClick={() => handleDelete(b.id, b.title)} className="p-2 text-foreground-light hover:text-destructive"><Trash2 className="w-4 h-4" /></button>
                            </td>
                        </tr>
                    ))}
                </tbody>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
                    <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-xl overflow-hidden relative">
                        <div className="p-5 border-b border-border flex justify-between items-center">
                            <h3 className="font-bold text-lg">{editingItem ? "Edit Book" : "Add Book"}</h3>
                        ...
                        <form onSubmit={handleSave} className="p-5 space-y-4">
                            ...
                            <div className="flex gap-4">
                                <div className="flex-1">
                                    ...
                                    <input type="text" value={formData.authorName} onChange={e => setFormData({ ...formData, authorName: e.target.value })} className="w-full p-2 border border-border rounded mt-1 bg-background" />
                                </div>
                                <div className="w-24">
