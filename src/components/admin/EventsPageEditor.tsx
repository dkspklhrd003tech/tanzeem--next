...
interface EventsPageEditorProps {
  pageId: string;
...
// ─── Main Component ──────────────────────────────────────────────────────────
export default function EventsPageEditor({ pageId, initialPageData }: EventsPageEditorProps) {
    ...
    try {
      const res = await fetch(`/api/sitemanager/pages/${pageId}`, {
        ...
        body: JSON.stringify({
          ...
          content: pageForm.content || "events Page",
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      setFormData({
        ...
        metaDescription: `event: ${cleanedTitle}`,
      });
      setFormErrors({});
      setIsModalOpen(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isNew ? "/api/admin/events" : `/api/admin/events/${editingItem.id}`;
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: err.message || "Failed to save the event.",
      });
    }
  };
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      toast({
        title: "Order Saved",
        description: "events grid sequence synchronized.",
      });
    } catch (err) {
      console.error(err);
      toast({
        ...
        <TabsContent value="list" className="space-y-6 outline-none">
          ...
          {isLoadingItems ? (
            ...
            <div className="bg-card rounded-xl border border-border p-12 text-center text-muted-foreground">
              {searchQuery ? "No search results match your criteria." : "No events found. Drop a image file above to add one!"}
            </div>
          ) : (
            <DndContext
              sensors={sensors}
          ...
          <div className="bg-card w-full max-w-2xl border border-border rounded-xl shadow-xl relative overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-border flex justify-between items-center bg-muted/20">
              <h2 className="text-xl font-bold flex items-center gap-2">
                ...
                {editingItem ? "Edit event Details" : "New event Details"}
              </h2>
              ...
              <form id="press-release-form" onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="e.g. event on Academic Achievements"
                  />
